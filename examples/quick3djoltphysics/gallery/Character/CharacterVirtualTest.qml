...
Item {
    ...
    readonly property vector3d cCharacterVelocity: Qt.vector3d(0, 0, 2)

    property real characterHeight: characterHeightStanding
    property real characterRadius: characterRadiusStanding
    ...
    function prePhysicsUpdate(frameDelta) {
        ...
        if (rampBlocksTimeLeft < 0.0) {
            for (var i = 0; i < 4; ++i) {
                ...
                rampBlock.setPositionAndRotation(position.plus(Qt.vector3d(2, 0, 0).times(i)), Qt.vector3d(-45, 0, 0))
        ...
        if (animatedCharacter.getGroundState() !== CharacterVirtual.OnGround)
            velocity = animatedCharacter.getLinearVelocity().times(animatedCharacter.up).plus(physicsSystem.gravity.times(frameDelta));
        velocity = velocity.plus(cCharacterVelocity.times(Math.sin(time)));

        animatedCharacter.setLinearVelocity(velocity);
        animatedCharacter.extendedUpdate(frameDelta,
                                 Qt.vector3d(0, -1, 0).times(physicsSystem.gravity.length()),
                                 updateSettings,
                                 moving,
                                 []);
    ...
    SampleCharacterCameraController {
        ...
        onCrouchActiveChanged: {
            if (characterController.crouchActive) {
                characterHeight = characterHeightCrouching
                characterRadius = characterRadiusCrouching;
            } else {
                characterHeight = characterHeightStanding
                characterRadius = characterRadiusStanding;
            }
            character.setShape(1.5 * 0.02, moving, []);
        }
    }
    ...
    View3D {
        ...
        RotatedTranslatedShape {
            ...
            position: Qt.vector3d(0, 0.5 * characterHeight + characterRadius, 0)
        }
        ...
        ExampleCharacterContactListener {
        ...
        Body {
            ...
            kinematicPosition: Qt.vector3d(0, 2.0, 15)
            Model {
                source: "#Cube"
                materials: PrincipledMaterial {
                    baseColorMap: Texture {
